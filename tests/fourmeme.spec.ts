...
test("Snapshot the whole Launch-your-token form (normalized)", async ({
  page,
}) => {
  ...
  if (await form.count()) {
    ...
    expect(normalizeOuterHtml(html)).toMatchSnapshot(
      "launch-form.outerHTML.txt"
    );
    return;
  }
  ...
  const container = heading.locator(
    "xpath=ancestor-or-self::*[.//input or .//textarea or .//select or .//button][1]"
  );
  await expect(container).toBeVisible();
  const html = await container.evaluate((el) => (el as HTMLElement).outerHTML);
  expect(normalizeOuterHtml(html)).toMatchSnapshot(
    "launch-container.outerHTML.txt"
  );
});
test("Snapshot Raised Token dropdown items", async ({ page }) => {
  ...
  const selectedInFirstDropdown = dropdown
    .locator('[aria-selected="true"]')
    .first();
  if ((await selectedInFirstDropdown.count()) === 0) {
    const firstOptionInFirstDropdown = dropdown
      .locator('[role="option"], [aria-selected]')
      .first();
  ...
  await expect(
    dropdown.locator('[aria-selected="true"]').first()
  ).toBeVisible();

  const html = await dropdown.evaluate((el) => (el as HTMLElement).outerHTML);
  expect(
    normalizeOuterHtml(html, {
      // HeadlessUI generates volatile ids (e.g. headlessui-listbox-option-:ru:)
      // which change between runs and cause noisy snapshot diffs.
      extraReplacements: [
        [/\bid="headlessui-[^"]+"/g, 'id="headlessui-<id>"'],
        [
          /\baria-activedescendant="headlessui-[^"]+"/g,
          'aria-activedescendant="headlessui-<id>"',
        ],
        [
          /\baria-controls="headlessui-[^"]+"/g,
          'aria-controls="headlessui-<id>"',
        ],
        [
          /\baria-labelledby="headlessui-[^"]+"/g,
          'aria-labelledby="headlessui-<id>"',
        ],
      ],
    })
  ).toMatchSnapshot("raised-token-dropdown.outerHTML.txt");
});
